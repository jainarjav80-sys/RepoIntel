...
let RepositoryService = class RepositoryService {
     ...
     */
    analyzeCommit(commitHash, repoPath) {
     ...
     */
    generateCommitSummary(commit, riskScore) {
     ...
     */
    generateRepositorySummary(repoPath) {
     ...
     */
    getRepositoryHealthScore(repoPath) {
        // Validate repository exists
        const fs = require('fs');
        if (!fs.existsSync(repoPath)) {
            throw new Error(`Repository path does not exist: ${repoPath}`);
        ...
        const overallHealthScore = Math.round((commitFrequencyScore * 0.25) +
