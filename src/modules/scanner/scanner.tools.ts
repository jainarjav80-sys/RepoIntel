import { ToolDecorator as Tool, Injectable, Inject, ExecutionContext, z } from '@nitrostack/core';
...
export class ScannerTools {
  ...
  async fetchRecentCommits(input: any, ctx: ExecutionContext) {
    try {
      // Validate input
  ...
  async scoreCommit(input: any, ctx: ExecutionContext) {
    try {
      // Validate input
